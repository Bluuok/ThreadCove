/**
 * Event Queue for the Async Generator Pattern (R10-owned component).
 *
 * Bridges async event handlers (.on() listeners) with AsyncGenerator<AgentEvent>.
 * Used by the Pi backend where events arrive asynchronously from a subprocess
 * JSONL stream, unlike the Claude backend's synchronous for-await loop over SDK
 * messages.
 *
 * Why this is not "an async generator with extra steps": the queue provides
 * push→pull decoupling (enqueue pushes and wakes waiters; drain pulls and
 * waits when empty), a completion signal (complete() ends the stream even
 * when waiters are blocked), and a lifecycle (reset() re-arms for the next
 * turn). A bare generator cannot be fed from a callback context.
 *
 * Pattern:
