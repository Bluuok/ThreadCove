 ...
 * Envelope codec (R12).
 *
 * JSON serialization with a type-annotation escape hatch for binary data:
 * Uint8Array values are encoded as base64 objects tagged with `__tcRpcType`.
 * Class identity is also lost across the wire — errors arrive as plain
 * WireError objects and receivers branch on `err.code`.
 */
import type { MessageEnvelope } from './types.ts';

const WIRE_TYPE_KEY = '__tcRpcType';
...
function isWireError(value: unknown): boolean {
  return isRecord(value) && value.code != null && typeof value.message === 'string';
}
